...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        network = trainer_class.build_network_architecture(plans_manager, dataset_json, configuration_manager,
                                                           1+output_channels*num_experts, enable_deep_supervision=False,num_experts=num_experts)
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            if self.perform_everything_on_gpu:
                try:
                    for params,params1,params2,params3,params4,params5 in zip(self.list_of_parameters,self.list_of_parameters1,self.list_of_parameters2,self.list_of_parameters3,self.list_of_parameters4,self.list_of_parameters5):
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Feature1,prediction1 = self.network1(x)
        Feature2,prediction2 = self.network2(x)
        Feature3,prediction3 = self.network3(x)
        Feature4,prediction4 = self.network4(x)
        Feature5,prediction5 = self.network5(x)
        
        coutput = torch.cat((x,Feature1[0], Feature2[0],Feature3[0], Feature4[0],Feature5[0]), dim=1)
        _,output = self.network(coutput)
        prediction = prediction1*output[:,:2,:,:,:]+prediction2*output[:,2:4,:,:,:]+prediction3*output[:,4:6,:,:,:]+prediction4*output[:,6:8,:,:,:]+prediction5*output[:,8:10,:,:,:]
