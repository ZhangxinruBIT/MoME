...
class nnUNetDataset(object):
    ...
    def load_case(self, key, load_mod=['t1','t1','t1','t1']):
        entry = self[key]
        if 'open_data_file' in entry.keys():
            data = entry['open_data_file']
            # print('using open data file')
        elif isfile(entry['data_file'][:-4] + ".npy"):
            # data = np.load(entry['data_file'][:-4] + ".npy", 'r') #(1, 160, 196, 160)
            # expanded_data = np.concatenate([data] * 4, axis=0)
            data_list = [np.load((entry['data_file'][:-4] + ".npy").replace(entry['data_file'][:-4].split('/')[-1].split('_')[2],mod), 'r') for idx,mod in enumerate(load_mod)]
        ...
        if 'open_seg_file' in entry.keys():
            ...
            seg = np.load((entry['data_file'][:-4] + "_seg.npy").replace('_t1ce_','_flair_'), 'r')
            if self.keep_files_open:
