...
from nnunetv2.training.nnUNetTrainer.Dispatch_network import DispatchNetwork1,ClsDispatchNet
...
class nnUNetTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.network = self.build_network_architecture(self.plans_manager, self.dataset_json,
                                                           ...
                                                           enable_deep_supervision=True,num_experts=num_experts,Decoder_only=False,Mod_Prior=True).to(self.device)
            self.network1 = self.build_network_architecture(self.plans_manager, self.dataset_json,
                                                           self.configuration_manager,
                                                           self.num_input_channels,
                                                           enable_deep_supervision=True).to(self.device)
            network1_pretrained_weights_file = '/gpfs/home/xz2223/Project/nnUNetV2/MoME_extension/multi-modality/MoME_with_multimod/Models/fold_BraTSt14/checkpoint_best.pth'
            ...
            self.network2 = self.build_network_architecture(self.plans_manager, self.dataset_json,
                                                           ...
                                                           self.num_input_channels,
                                                           enable_deep_supervision=True).to(self.device)
            network2_pretrained_weights_file = '/gpfs/home/xz2223/Project/nnUNetV2/MoME_extension/multi-modality/MoME_with_multimod/Models/fold_BraTSt1ce4/checkpoint_best.pth'
            ...
            self.network3 = self.build_network_architecture(self.plans_manager, self.dataset_json,
                                                           ...
                                                           self.num_input_channels,
                                                           enable_deep_supervision=True).to(self.device)
            network3_pretrained_weights_file = '/gpfs/home/xz2223/Project/nnUNetV2/MoME_extension/multi-modality/MoME_with_multimod/Models/fold_BraTSt24/checkpoint_best.pth'
            ...
            self.network4 = self.build_network_architecture(self.plans_manager, self.dataset_json,
                                                           ...
                                                           self.num_input_channels,
                                                           enable_deep_supervision=True).to(self.device)
            network4_pretrained_weights_file = '/gpfs/home/xz2223/Project/nnUNetV2/MoME_extension/multi-modality/MoME_with_multimod/Models/fold_BraTSflair4/checkpoint_best.pth'
            load_pretrained_weights(self.network4, network4_pretrained_weights_file, verbose=True)
            
            self.dispatch_network = ClsDispatchNet(input_dim=num_experts).to(self.device)#DispatchNetwork1(input_dim=num_experts).to(self.device)
            # self.network5 = self.build_network_architecture(self.plans_manager, self.dataset_json,
            #                                                self.configuration_manager,
            #                                                self.num_input_channels*num_experts,
            #                                                enable_deep_supervision=True).to(self.device)
            # network5_pretrained_weights_file = ''
    ...
    def build_network_architecture(plans_manager: PlansManager,
                                   ...
                                   num_input_channels,
                                   enable_deep_supervision: bool = True,num_experts=1,Decoder_only=False,Mod_Prior=False) -> nn.Module:
        """
        ...
        return get_network_from_plans(plans_manager, dataset_json, configuration_manager,
                                      num_input_channels, deep_supervision=enable_deep_supervision,num_experts=num_experts,Decoder_only=Decoder_only,Mod_Prior=Mod_Prior)
    ...
    def configure_optimizers(self):
        ...
        all_parameters = list(self.network1.parameters()) + list(self.network2.parameters()) + list(self.network3.parameters()) + list(self.network4.parameters()) + list(self.dispatch_network.parameters()) + list(self.network.parameters())
    ...
    def do_split(self):
        ...
        elif self.fold == 'BraTS_MoME_default' or self.fold == 'BraTS_MoME_defaultV1':
    ...
    def on_train_end(self):
        ...
        if self.local_rank == 0 and isfile(join(self.output_folder, "checkpoint_latest_dispatch.pth")):
            os.remove(join(self.output_folder, "checkpoint_latest.pth"))
            os.remove(join(self.output_folder, "checkpoint_latest1.pth"))
            os.remove(join(self.output_folder, "checkpoint_latest2.pth"))
            os.remove(join(self.output_folder, "checkpoint_latest3.pth"))
            os.remove(join(self.output_folder, "checkpoint_latest4.pth"))
    ...
    def train_step(self, batch: dict,r) -> dict:
        data = batch['data'].to(self.device)#[:,:,32:96,32:96,32:96]
        target = batch['target']
        sup_losses = batch['sup_loss']
        
        mask_modality = torch.tensor(sup_losses).to(self.device)*1.0
        ...
        '''
        OptionONE------Prior Constrain: 1.mask 2.avaliable channel
        '''     
        # sparse_mask_dispatch_embading = dispatch_embading*modality_embading.to(self.device)
        # mask_dispatch_embading = torch.pow(sparse_mask_dispatch_embading, 10) # (b,4,4)
        # mask_dispatch_embading /= mask_dispatch_embading.sum(dim=-1, keepdim=True) # (b,4,4)
        # B, M = mask_dispatch_embading.shape[:2]
        # c_mask = torch.eye(M)[None,...].repeat(B,1,1).to(mask_dispatch_embading)
        # keep_modality = mask_modality == 1
        # mask_dispatch_embading[keep_modality,:] = c_mask[keep_modality,:]  # mask_dispatch_embading (argmx) shape(2,4)
        # dispatched_data = torch.sum(mask_dispatch_embading[...,None, None, None] * data.unsqueeze(1), dim=2)
        '''
        ...
        # Mix wight sum to 1
        # sum_values = torch.sum(sparse_mask_dispatch_embading, dim=2, keepdim=True)
        # sparse_mask_dispatch_embading = sparse_mask_dispatch_embading/(sum_values+1e-8)
        # sparse_mask_dispatch_embading = sparse_mask_dispatch_embading*modality_embading
        dispatched_data = torch.sum(sparse_mask_dispatch_embading[...,None, None, None] * data.unsqueeze(1), dim=2)
        '''
        OptionTWO------None Constrain
        ''' 
        # dispatch_embading = torch.pow(dispatch_embading, 100)
        # dispatched_data = torch.sum(dispatch_embading[...,None, None, None] * data.unsqueeze(1), dim=2)
        # self.print_to_log_file(batch['keys'],sup_losses[0],dispatch_embading[0])
        
        data = dispatched_data.to(self.device, non_blocking=True)
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            Feature1,output1 = self.network1(data[:,0].unsqueeze(1))
            Feature2,output2 = self.network2(data[:,1].unsqueeze(1))
            Feature3,output3 = self.network3(data[:,2].unsqueeze(1))
            Feature4,output4 = self.network4(data[:,3].unsqueeze(1))
            # Feature5,output5 = self.network5(data)
            
            coutput = torch.cat((data,Feature1[0], Feature2[0],Feature3[0], Feature4[0]), dim=1)  #(2,1+32+32,128,128,128)
            # coutput_debug = torch.cat((output1[0], output2[0]), dim=1)
            _,output = self.network([coutput,sup_losses])
            classnum = 4
            Final_output = [output1[i]*output[i][:,:classnum,:,:,:]+output2[i]*output[i][:,classnum:2*classnum,:,:,:]+output3[i]*output[i][:,2*classnum:3*classnum,:,:,:] \
                +output4[i]*output[i][:,3*classnum:4*classnum,:,:,:] for i in range(len(output))]
            # Final_output = sup_losses[0]*[output1[i]*output[i][:,:classnum,:,:,:]+sup_losses[1]*output2[i]*output[i][:,classnum:2*classnum,:,:,:]+sup_losses[2]*output3[i]*output[i][:,2*classnum:3*classnum,:,:,:] \
            #     +sup_losses[3]*output4[i]*output[i][:,3*classnum:4*classnum,:,:,:] for i in range(len(output))]
            output = Final_output
            # # del data
            # l1 = 0
            outputs = [output1, output2, output3, output4]
            # for bdata,i in enumerate(batch['data']):
            #     if 'healthy' in batch['keys'][i]:
            ...
            #         sub_output = [o[num:num+1] for o in output4]
            #     l1 += self.loss(sub_output, sub_target)
            
            l1 = 0    
            for num, mod_list in enumerate(sup_losses):
                sub_target = [t[num:num+1] for t in target] #select the target within batch
                sub_lt = 0
                for idx,c in enumerate(mod_list):
                    if c == 1:
                        sub_output = [o[num:num+1] for o in outputs[idx]]
                        sub_lt = sub_lt + self.loss(sub_output, sub_target)
                sub_lt = sub_lt/sum(mod_list)
                l1 = l1 + sub_lt
        ...
        if self.grad_scaler is not None:
            ...
            torch.nn.utils.clip_grad_norm_(self.network3.parameters(), 12)
            torch.nn.utils.clip_grad_norm_(self.network4.parameters(), 12)
            torch.nn.utils.clip_grad_norm_(self.dispatch_network.parameters(), 12)
    ...
    def validation_step(self, batch: dict) -> dict:
        data = batch['data'].to(self.device,non_blocking=True)#[:,:,32:96,32:96,32:96]
        target = batch['target']
        sup_losses = batch['sup_loss'] #(b,4)
        
        mask_modality = torch.tensor(sup_losses).to(self.device)*1.0
        modality_embading = mask_modality[...,None].repeat(1,1,4).permute(0,2,1) # M(b,4,4)
        # for b in range(data.shape[0]):
        #     for c in range(data.shape[1]):
        #         if mask_modality[b,c] == 0:
        #             # data[b,c] = torch.randn(data.shape[2:], device='cuda') #miss modality using the Gaussian distribution instead
        #             data[b,c] = torch.zeros(data.shape[2:], device='cuda')
        ...
        # OptionONE------Prior Constrain: 1.mask 2.avaliable channel
        # '''
        # sparse_mask_dispatch_embading = dispatch_embading*modality_embading.to(self.device) #Mask the dispatch possibility for the missing modality channel
        # mask_dispatch_embading = torch.argmax(sparse_mask_dispatch_embading, dim=2) #(b,4) #Argmax only apply to the channel with the avalible modality so that avoid assigne the missed Gaussian distribution instead data
        # # index = mask_dispatch_embading[:,:,None,None,None].expand(-1,-1,*data.shape[2:]).to(self.device)  # (b,4,64,64,64)
        # dispatched_data = torch.zeros_like(data)
        # for b in range(data.shape[0]):
        #     for c in range(data.shape[1]):
        #         dispatched_data[b,c] = data[b,mask_dispatch_embading[b,c]]
        # dispatched_data[mask_modality == 1] = data[mask_modality == 1]
        '''
        OptionONE------Prior Constrain-TOPK: 1.mask 2.avaliable channel
        '''     
        sparse_mask_dispatch_embading = dispatch_embading*modality_embading.to(self.device) #(2,4,128,128,128)
        ...
        # dispatch_embading = torch.pow(dispatch_embading, 100)
        # dispatched_data = torch.sum(dispatch_embading[...,None, None, None] * data.unsqueeze(1), dim=2)
        if sum(sup_losses[0])>1:
            self.print_to_log_file(batch['keys'],sup_losses[0],sparse_mask_dispatch_embading[0])
        ...
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                if self.is_ddp:
                    ...
                    mod3 = self.network3
                    mod4 = self.network4
                    dispatch_mod = self.dispatch_network
                    # mod5 = self.network5
                if isinstance(mod, OptimizedModule):
                    ...
                    mod3 = mod3._orig_mod
                    mod4 = mod4._orig_mod
                    dispatch_mod = dispatch_mod._orig_mod
                    # mod5 = mod2._orig_mod
                ...
                checkpoint = {
                    'network_weights': dispatch_mod.state_dict(),
                    # 'optimizer_state': self.optimizer.state_dict(),
                    # 'grad_scaler_state': self.grad_scaler.state_dict() if self.grad_scaler is not None else None,
                    # 'logging': self.logger.get_checkpoint(),
                    # '_best_ema': self._best_ema,
                ...
                torch.save(checkpoint, filename.replace('best','best_dispatch').replace('latest','latest_dispatch').replace('final','final_dispatch'))   
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        if isinstance(filename_or_checkpoint, str):
            ...
            checkpoint_dispatch = torch.load(filename_or_checkpoint.replace('latest','latest_dispatch').replace('best','best_dispatch'), map_location=self.device)
        ...
        new_state_dict_dispatch = {}
        for k, value in checkpoint_dispatch['network_weights'].items():
            key = k
            # if key not in self.dispatch_network.state_dict().keys() and key.startswith('module.'):
            #     key = key[7:]
            new_state_dict_dispatch[key] = value
        ...
        if self.is_ddp:
            ...
            if isinstance(self.network, OptimizedModule):
                ...
                self.dispatch_network.load_state_dict(new_state_dict_dispatch)
